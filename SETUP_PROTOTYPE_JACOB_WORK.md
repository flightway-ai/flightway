...
- **Leo_Working** — Career Hub map (`dashboard.html`), quiz → hub email flow, career advisor trigger
...
| **Talk to career advisor** | Opens `Flightway.html#coach` with quiz context in `localStorage` |
| **Sign in (no quiz)** | `Flightway.html#signin` → portal with Career Hub + advisor links |
...
3. **Talk to career advisor** → lands on AI coach in `Flightway.html#coach` (no home-page flash).
4. **Sign in from landing** → `Flightway.html#signin` → portal without taking the quiz.
5. **Email me my link** → email contains `dashboard.html#r=…`; link opens Career Hub.
6. Coach signup/chat works (`POST /account`, `POST /chat`).
